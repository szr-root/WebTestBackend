...
# 查询测试套件列表
@router.get('/suites', tags=['测试套件管理'], summary='查询测试套件列表', status_code=200, response_model=List[SuiteSchemas])
async def get_suites(project_id: int | None = None, modules_id: int | None = None):
    query = Suite.all()
...
async def copy_cases(id: int):
    ...
    if not cases:
        raise HTTPException(status_code=404, detail='用例不存在')
    new_cases = await Cases.create(name=cases.name, project=cases.project, steps=cases.steps)
    return new_cases


# 查询测试用例列表
@router.get('/cases', tags=['测试用例管理'], summary='查询测试用例列表', status_code=200, response_model=List[CasesListSchema])
async def get_cases(project_id: int):
    cases = await Cases.filter(project=project_id)
    result = [CasesListSchema(**case.__dict__) for case in cases]
    return result
